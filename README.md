# tic_tac_toe

I made this mobile game using Dart/Flutter.
I used Firebase for authentication and storing data.

## Login Page
![LoginView](https://github.com/ertugrulkoca/tic_tac_toe_FLUTTER/blob/main/login.png)

## Google Sign
![GoogleSign](https://github.com/ertugrulkoca/tic_tac_toe_FLUTTER/blob/main/googleSign.png)

## Game Page
![GameView](https://github.com/ertugrulkoca/tic_tac_toe_FLUTTER/blob/main/gameView.png)

## Win Page
![winView](https://github.com/ertugrulkoca/tic_tac_toe_FLUTTER/blob/main/win.png)
